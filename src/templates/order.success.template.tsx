...
export default function OrderSuccess({ user, order }: Props) {
    //Get cartId
    const cartStorage = localStorage.getItem('cart_id');
    //If there's a cart ID
    if (cartStorage) {
        //Delete it after successful order
        localStorage.removeItem('cart_id');
    }

    return (
        <React.Fragment>
            <div className='p-5 rounded-md bg-green-50 border border-green-300 mb-7'>
                <h1 className='mb-5'>Merci pour votre commande {user.name} !</h1>
                <p>Votre commande à bien été validée. Vous trouverez le détail ci-dessous.</p>
                <p>Vous pourrez suivre l'avancée de votre commande <Link className="text-red-500" href={`/user/orders/${order._id}`}>ici</Link>.</p>
            </div>
