...
export const isAuthenticated = (uid, link) => uid ? link : `/login/?from=${link}`
...
export const getMinBreakpoints = (breakpoint) => {
    switch (breakpoint) {
        case 'xs':
            return 0
        case 'sm':
...
/**
 * Test if a password contains at least :
...
/**
 * Check if string contains only letters, spaces, dashes (-) and dots (.)
...
/**
 * Check if string is a valid pathname. Ex: /valid-path/name
 * @param {*} string String to check
 */

export const isValidPathname = (string) => {
    // eslint-disable-next-line
...
/**
 * Check if string is a valid postcode. Ex: 39190
 * @param {*} string String to check
 */
...
/**
 * Add spaces between numbers: 100, 1 000, 10 000, 100 000, 1 000 000...
 * @param {*} x Number
 */
 ...
 * @param {*} property property that contain numbers
 * @param {*} trailing float length
 */

export const getTotal = (array, property, trailing) => {
    let total = 0
    if (property) {
...
export const getPastMonths = (nbOfMonthsPast) => {
    ...
    for (var i = nbOfMonthsPast; i > 0; i -= 1) {
        d = new Date(today.getFullYear(), today.getMonth() - i, 1);
        pastMonths = [...pastMonths, months[d.getMonth()]];
    }
}
...
/**
 * Exerce a deep comparison between two objects recursivly. Return true if equal, false otherwise
 * @param {*} x Original object to compare with
 * @param {*} y Object to compare with the original
 */
...
export function areObjectsEqual(x, y) {
    ...
    var p = Object.keys(x);
    return Object.keys(y).every((i) => { return p.indexOf(i) !== -1; }) && p.every((i) => { return areObjectsEqual(x[i], y[i]); });
}

/**
 * Return the fields that are differents between two objects
 * @param {*} original First object
 * @param {*} newObj Second object
 */

export const getUpdatedObjectFields = (original, newObj) => {
    let differences = {}
    const o = original
    const n = newObj
    ...
    Object.keys(n).forEach((key) => {
        if (o[key]) {
            let areEqual = areObjectsEqual(o[key], n[key])
            if (!areEqual) {
                differences[key] = n[key]
            }
        } else {
            differences[key] = n[key]
        }
...
export const handleObjectProp = (array, key, property, value) => {
    let arr = [...array]
 ...
 * @param {*} array Array to add item in or remove item from
 * @param {*} item Item to add or remove
 * @param {*} property property used to filter
 * @param {*} value property value to find to check if item is already in array
 */

export const addOrRemoveObject = (array, item, property) => {
    let index = array.findIndex(el => el[property] === item[property])
    if (index !== -1) {
        let arr = [...array]
        arr.splice(index, 1)
        return arr
...
/**
 * Remove all duplicated objects in an array based on the property parameter
 * @param {*} array Array to remove from
 * @param {*} property Property used to filter duplication
 */


export const removeDuplicatedObjects = (array, property) => {
    const result = Object.values(
        array.reduce((acc, obj) => ({ ...acc, [obj[property]]: obj }), {})
    );
    return result
}
...
/**
 * Sort datas from property based on `propertiesArr` order
 * @param {*} datas Datas array
 * @param {*} property Property used to sort
 * @param {*} propertiesArr Property values used to sort.
 ** Ex : `['Failed', 'In progress', 'Pending', 'Success']`
 */

export const sortByProperty = (datas, property, propertiesArr) => {
    return [...datas].sort((a, b) => {
        return propertiesArr.indexOf(a[property]) - propertiesArr.indexOf(b[property]);
    });
}
 ...
 * @param {*} order Direction to sort : `asc` or `desc`
 * @param {*} property Name of the date property
 */

export const sortByDate = (datas, order, property) => {
    if (order === 'asc') {
        return [...datas].sort((a, b) => { return new Date(b[property]) - new Date(a[property]) })
    } else {
        return [...datas].sort((a, b) => { return new Date(a[property]) - new Date(b[property]) })
    }
}

/**
 * Sort datas from numbers
 * @param {*} datas Datas to sort
 * @param {*} order Direction to sort : `asc` or `desc`
 * @param {*} property Name of the date property
 */

export const numericSort = (datas, order, property) => {
    if (order === 'asc') {
        return [...datas].sort((a, b) => { return a[property] - b[property] })
    } else {
        return [...datas].sort((a, b) => { return b[property] - a[property] })
    }
}
...
export function multiplyArray(array, num) {
    var newArr = [];
    for (let i = 0; i < num; [i++].push.apply(newArr, array));
    return newArr;
}
